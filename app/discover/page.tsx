...
function DiscoverClient() {
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('*').limit(100).then(({ data, error }) => {
      if (!active) return;
      setProfiles(error? [] : (data || []).map(normalizeProfile));
      setLoading(false);
  ...
  const handleAction = async (profile: Profile, type: string) => {
    ...
    if (type === 'pass') return setMessage(`Passed on ${profile.name}`);
  ...
  <div className="app-shell w-full max-w-[480px] bg-[#fbf9ff] min-h-screen relative pb-[88px]">
    {/* PRO TOPBAR - blur + border */}
    <header className="topbar sticky top-0 z-30 bg-[#fbf9ff]/80 backdrop-blur-xl border-b border-black/[0.06] px-5 py-3 flex items-center justify-between">
      <div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button className="h-9 w-9 grid place-items-center rounded-full bg-white border border-black/10 shadow-sm" aria-label="Open filters">⌁</button>
      </div>
    </header>

    <main className="content animate-page px-4">
      {/* HERO - same text, pro gradient + small decoration */}
      <section className="relative mt-4 rounded-[28px] bg-[#111] p-6 text-white shadow-xl overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="mt-3 text-[13px] leading-[18px] text-white/70 max-w-[260px]">Real connections across countries, cultures and communities.</p>
        </div>
      </section>

      {/* MODES - same pills, pro scroll + active = black */}
      <div className="mt-5 -mx-4 px-4 flex gap-2 overflow-x-auto scrollbar-none pb-1">
        {modes.map(([value, label]) => (
          <button key={value} onClick={() => setMode(value)}
            className={`shrink-0 min-w-max rounded-full px-4 py-2.5 text-[12px] font-bold transition-all border ${mode === value? 'bg-black text-white border-black shadow-[0_4px_12px_rgba(0,0,0,0.2)] scale-[1.02]' : 'bg-white text-zinc-600 border-zinc-200 hover:border-zinc-300'}`}>
            {label}
          </button>
        ))}
      </div>

      {/* COUNTRY - same select, pro */}
      <div className="mt-3">
        <select value={country} onChange={e => setCountry(e.target.value)}
          className="w-full h-[44px] rounded-full bg-white border border-zinc-200 px-4 text-[13px] font-medium outline-none focus:border-black">
      ...
      {loading? (
        <div className="mt-6 rounded-[20px] bg-white border border-black/5 p-8 text-center shadow-sm">
          <div className="mx-auto h-10 w-10 rounded-full bg-zinc-100 animate-pulse" />
          <p className="mt-3 text-[13px] font-medium text-zinc-500">Finding people around the world...</p>
        </div>
      ) : filtered.length? (
        <>
          <section className="mt-6">
            <div className="mb-3 flex items-end justify-between px-1">
              <div>
                <p className="text-[10px] font-black tracking-[2px] text-violet-600">{mode === 'worldwide'? 'WORLDWIDE' : modes.find(([v]) => v === mode)?.[1].toUpperCase()}</p>
                <h2 className="text-[18px] font-black tracking-[-0.5px] mt-1">People you may like</h2>
              </div>
              <span className="text-[11px] font-bold text-zinc-400 bg-zinc-100 px-2.5 py-1 rounded-full">{filtered.length}</span>
            </div>
            {/* SMALL photo card container - 420px max, not fullscreen */}
            <div className="mx-auto max-w-[420px] rounded-[26px] overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.15)] border border-black/5">
              <ProfileCard profile={filtered[0]} onAction={type => handleAction(filtered[0], type)} />
            </div>
          </section>

          <div className="mt-8 space-y-6">
            <SectionRow title="Best Matches" icon="❤️" profiles={profiles.slice(0, 8)} />
            <SectionRow title="Around the World" icon="🌍" profiles={profiles.filter(profile => profile.country).slice(0, 8)} />
            <SectionRow title="Near You" icon="📍" profiles={near} />
            <SectionRow title="Online Now" icon="🟢" profiles={online} />
            <SectionRow title="New Members" icon="🆕" profiles={newMembers} />
          </div>
        </>
      ) : (
        <section className="mt-8 rounded-[24px] bg-white border border-black/5 p-8 text-center shadow-sm">
          <div className="text-4xl">🌍</div>
          <h2 className="mt-4 text-[20px] font-black">You've explored this view</h2>
          <p className="mt-2 text-[13px] leading-[18px] text-zinc-500">Try Worldwide Discovery, choose another country, or broaden your preferences.</p>
          <div className="mt-5 flex justify-center gap-2">
            <button onClick={() => { setMode('worldwide'); setCountry('') }} className="rounded-full bg-black px-5 py-3 text-[12px] font-black text-white">Explore worldwide</button>
            <a href="/profile" className="rounded-full border border-zinc-200 px-5 py-3 text-[12px] font-bold">Improve profile</a>
          </div>
        </section>
      )}

      {message && (
        <div className="fixed bottom-[90px] left-1/2 -translate-x-1/2 z-50 bg-black text-white px-4 py-2.5 rounded-full text-[12px] font-bold shadow-xl">
          {message}
        </div>
      )}
    </main>
