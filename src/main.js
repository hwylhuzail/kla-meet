...
const footerHTML=`<footer class="bg-black text-zinc-400 text-[11px] text-center py-6 mt-12">© KLA-MEET • <a href="/login" onclick="route(event,'/login')" class="underline font-bold">/login</a> • <a href="/premium" onclick="route(event,'/premium')" class="underline font-bold">/premium</a></footer>`
...
document.getElementById('app').innerHTML=`<nav class="bg-black text-white px-6 py-4 flex justify-between"><div class="font-black">KLA•MEET</div><button onclick="route(event,'/login')" class="bg-yellow-400 text-black px-5 py-1.5 rounded-full font-bold text-sm">/login</button></nav><section class="px-6 py-8"><h1 class="text-5xl font-black">Date. Meet.<br>Connect.</h1><button onclick="renderOnboarding()" class="mt-6 bg-yellow-400 px-8 py-3.5 rounded-full font-black">Get Started →</button><div class="flex gap-2 mt-4"><button onclick="route(event,'/login')" class="text-xs bg-black text-white px-4 py-2 rounded-full">/login subfolder</button><button onclick="route(event,'/premium')" class="text-xs bg-yellow-400 px-4 py-2 rounded-full font-bold">$3.99</button></div></section>${footerHTML}`
}
function renderOnboarding(){
let h= step==1? `<div class="min-h-screen flex items-center justify-center p-6"><div class="w-full max-w-[360px] border-2 border-black rounded-[30px] p-7 bg-white"><h2 class="font-black text-xl text-center">Create Account</h2><input id="name" placeholder="Full name" class="w-full mt-5 border-2 border-black rounded-xl px-4 py-3 text-sm"><input id="email" placeholder="Email" class="w-full mt-3 border rounded-xl px-4 py-3 text-sm"><input id="pass" type="password" placeholder="Password min 6" class="w-full mt-3 border rounded-xl px-4 py-3 text-sm"><button onclick="signUp()" class="mt-5 w-full bg-yellow-400 py-3.5 rounded-full font-black">Create + Send Code</button><p class="text-xs text-center mt-3"><a href="/login" onclick="route(event,'/login')" class="underline font-bold">Login /login</a></p></div></div>` : `<div class="min-h-screen flex items-center justify-center p-6"><div class="w-full max-w-[360px] border rounded-[30px] p-7 text-center bg-white"><div class="text-4xl">📧</div><h2 class="font-black mt-3">Check Email — Code Sent to ${onboard.email}</h2><input id="otp" placeholder="6-digit code" class="w-full mt-4 border-2 border-black rounded-xl px-4 py-3 text-center tracking-[6px]"><button onclick="verifyOtp()" class="mt-3 w-full bg-black text-white py-3.5 rounded-full font-bold">Verify →</button><button onclick="resendCode()" class="mt-3 w-full bg-zinc-100 py-3 rounded-full text-xs">Resend</button></div></div>`
