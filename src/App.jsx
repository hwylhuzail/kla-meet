        ...
        <div className="fixed inset-0 bg-black/90 p-4 flex items-center justify-center z-[200]">
          <div className="bg-zinc-900 rounded-[24px] overflow-hidden w-full max-w-sm border border-[#FFC300]/30">
            ...
            <div className="p-4">
              ...
              <p className="text-[11px] text-white/60">📍 {selectedPost.city}</p>
      ...
      {showPostModal && (
        <div className="fixed inset-0 bg-black/95 p-0 flex items-end justify-center z-[100]">
          <div className="bg-zinc-900 rounded-t-[32px] p-6 w-full max-w-md max-h-[90vh] overflow-y-auto border-t border-[#FFC300]/20">
            ...
            <div className="mt-4"><input type="file" accept="image/*" onChange={handleImage} className="mt-2 w-full text-xs file:bg-white file:text-black file:rounded-full file:px-4 file:py-2 file:text-xs file:font-bold" />{postForm.preview && <img src={postForm.preview} className="mt-3 w-full h-64 object-cover rounded-[20px]" />}</div>
            <textarea value={postForm.bio} onChange={e=>setPostForm({...postForm,bio:e.target.value})} placeholder="Here looking for a serious marriage partner" className="mt-4 w-full bg-black border border-white/20 rounded-2xl px-4 py-3 text-xs h-20" />
            <input value={postForm.interests} onChange={e=>setPostForm({...postForm,interests:e.target.value})} placeholder="music" className="mt-2 w-full bg-black border border-white/20 rounded-full px-4 py-3 text-xs" />
            <div className="mt-3 bg-black rounded-2xl p-4 border border-white/10"><div className="flex justify-between items-center"><p className="text-xs font-bold">📍 Location {postForm.lat? <span className="text-green-400">• True</span> : null}</p><button onClick={getLocation} className="bg-[#FFC300] text-black px-5 py-2.5 rounded-full text-[11px] font-black">True Location</button></div><p className="text-[10px] text-white/60 mt-2">City: {postForm.city}</p></div>
            <button onClick={handleCreatePost} disabled={posting} className="mt-5 w-full bg-[#FFC300] text-black rounded-full py-4 font-black text-[15px]">{posting? 'Posting...' : 'Post Now • Keep Love Alive'}</button>
          </div>
        </div>
      )}
