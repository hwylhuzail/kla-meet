  ...
  const handlePesapal = async () => {
    try{
      const r = await fetch('/api/pesapal', {
        ...
        body: JSON.stringify({plan: price==='2.99'?'basic':'standard'})
      })
      ...
      // NO [object Object] anymore - standalone crypto
      window.open(OXA, '_blank')
    }
  }
  return (
    ...
    return (
      <div className="min-h-screen bg-white text-black">
        <header className="bg-black text-white px-4 py-3 flex justify-between">
          <h1 className="font-black text-sm">KLA-MEET • Keep Love Alive</h1>
          <button onClick={()=>{setView('onboarding'); setOnStep(1)}} className="bg-[#FFC300] text-black px-4 py-2 rounded-full font-bold text-xs">Get Started →</button>
        </header>
        <section className="px-6 py-10 grid md:grid-cols-2 gap-6">
          <div><h2 className="text-[40px] font-black leading-[0.9]">Date. Meet.<br/>Connect.<br/>Worldwide.</h2></div>
          <div className="grid grid-cols-3 gap-2">{WORLD.slice(0,3).map(w=><img key={w.city} src={w.img} className="h-28 rounded-xl object-cover" />)}</div>
        </section>
        <section className="bg-[#f7f7f6] px-6 py-8">
          <h3 className="text-center font-black">How KLA-MEET Works — Premium • Keep Love Alive</h3>
          <p className="text-center text-[11px] text-zinc-500">Worldwide priority. 5 steps. Each button stands alone.</p>
          <div className="flex gap-3 overflow-x-auto mt-6 pb-4">
            <PremiumCard step={1} title="What should we call you?" price="2.99" />
            <PremiumCard step={2} title="Your gender" price="2.99" />
            <PremiumCard step={3} title="What are you looking for?" price="5.99" />
            <PremiumCard step={4} title="Pick your interests" price="5.99" />
            <PremiumCard step={5} title="Bio + Location" price="5.99" />
          </div>
        </section>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white pb-24">
      ...
      {tab==='discover' && (
        <div className="max-w-md mx-auto p-4">
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            {WORLD.map(w=><div key={w.city} className="bg-zinc-900 rounded-[20px] overflow-hidden"><img src={w.img} className="h-32 w-full object-cover"/><div className="p-2"><p className="text-xs font-bold">{w.flag} {w.city}</p><p className="text-[9px] text-white/50">Keep Love Alive</p></div></div>)}
          </div>
        </div>
      )}
      {tab==='premium' && (
        <div className="max-w-md mx-auto p-4">
          ...
          <div className="mt-4 space-y-4">
            <div className="bg-[#FFC300] text-black rounded-2xl p-4">
              ...
              <button onClick={()=>window.open(OXA,'_blank')} className="w-full bg-black text-white rounded-full py-3 font-bold text-xs mt-2">⚡ Crypto OXA 18802533 • $2.99</button>
              <p className="text-[9px] mt-1">Wallet {WALLET}</p>
            </div>
            <div className="bg-zinc-900 rounded-2xl p-4 border border-white/10">
              ...
              <button onClick={async()=>{
                try{
                  const r=await fetch('/api/pesapal',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({plan:'basic'})})
                  const j=await r.json(); if(j.redirect_url) window.open(j.redirect_url,'_blank'); else alert('Pesapal keys missing, use Crypto')
                }catch{ window.open(OXA,'_blank') }
              }} className="w-full bg-[#FF6A00] text-white rounded-full py-3 font-bold text-xs mt-2">Pesapal MTN/Airtel/Card • $2.99</button>
            </div>
            <div className="bg-[#FFC300] text-black rounded-2xl p-4">
              <p className="font-black text-sm">Standard $5.99 — Crypto ALONE</p>
              <button onClick={()=>window.open(OXA,'_blank')} className="w-full bg-black text-white rounded-full py-3 font-bold text-xs mt-2">⚡ Crypto OXA 18802533 • $5.99</button>
            </div>
            <div className="bg-zinc-900 rounded-2xl p-4 border border-white/10">
              <p className="font-black text-sm">Standard $5.99 — Pesapal ALONE</p>
              <button onClick={async()=>{
                try{
                  const r=await fetch('/api/pesapal',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({plan:'standard'})})
                  const j=await r.json(); if(j.redirect_url) window.open(j.redirect_url,'_blank'); else alert('Pesapal keys missing, use Crypto')
                }catch{ window.open(OXA,'_blank') }
              }} className="w-full bg-[#FF6A00] text-white rounded-full py-3 font-bold text-xs mt-2">Pesapal MTN/Airtel/Card • $5.99</button>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-black border-t border-white/10 flex justify-around py-3">
        ...
        <button onClick={()=>setTab('premium')} className={`text-[11px] ${tab==='premium'?'text-[#FFC300] font-black':''}`}>★ Premium</button>
      </nav>
    </div>
  )
}
