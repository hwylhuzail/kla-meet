  ...
  if(view==='landing'){
    return (
      <div className="min-h-screen bg-white text-black">
        ...
        {showPrivacy && (<div className="fixed inset-0 bg-black/90 z-[300] p-4 overflow-y-auto"><div className="bg-white rounded-[24px] p-6 max-w-md mx-auto"><h2 className="font-black text-sm">Privacy Policy</h2><div className="mt-4 text-[11px] space-y-2 leading-relaxed"><p><b>Data:</b> name, email, age, gender, bio, city, photos. Used only to provide service.</p><p><b>Security:</b> Encrypted, never sold.</p><p><b>Location:</b> City-level only.</p><p><b>Rights:</b> Delete anytime via support. GDPR compliant.</p><p><b>Contact:</b> support@kla-meet.com</p><p><b>Age:</b> 18+ only.</p></div><button onClick={()=>{setAgreed(true); setShowPrivacy(false)}} className="mt-4 w-full bg-black text-white rounded-full py-3 font-black text-xs">I Agree</button></div></div>)}
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white pb-28">
      ...
      {tab==='profile' && (
        <div className="max-w-md mx-auto p-4 space-y-4">
          ...
          <p className="text-[10px] text-white/60">{user?.email} • Secure • Verified {isPremium?'• PREMIUM':''}</p>
          <div className="bg-zinc-900 rounded-[24px] p-5 space-y-3 border border-white/10">
            <h3 className="font-black text-xs text-[#FFC300]">✏️ Edit Your Information</h3>
            <div><p className="text-[10px] text-white/50 mb-1">Display Name</p><input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Your name" className="w-full bg-black border border-white/20 rounded-full px-4 py-3 text-xs" /></div>
            <div className="flex gap-2">
              ...
              <div className="w-1/2"><p className="text-[10px] text-white/50 mb-1">Age</p><input value={form.age} onChange={e=>setForm({...form,age:e.target.value})} placeholder="22" className="w-full bg-black border border-white/20 rounded-full px-4 py-3 text-xs" /></div>
            </div>
            <div><p className="text-[10px] text-white/50 mb-1">About Me / Bio</p><textarea value={form.bio} onChange={e=>setForm({...form,bio:e.target.value})} placeholder="Tell people about you..." className="w-full bg-black border border-white/20 rounded-2xl px-4 py-3 text-xs h-20" /></div>
            <div><p className="text-[10px] text-white/50 mb-1">Interests</p><input value={form.interests} onChange={e=>setForm({...form,interests:e.target.value})} placeholder="music, travel, love" className="w-full bg-black border border-white/20 rounded-full px-4 py-3 text-xs" /></div>
            <button onClick={async()=>{
              if(!user) return
              const { error } = await supabase.from('profiles').update({name:form.name, bio:form.bio, interests:form.interests, gender:form.gender, age:form.age}).eq('id',user.id)
              if(error) alert(error.message)
              else { alert('Profile saved ✓'); fetchProfiles() }
            }} className="w-full bg-[#FFC300] text-black rounded-full py-3 font-black text-xs mt-2">Save Profile ✓</button>
          </div>
          <div className="bg-black border border-white/10 rounded-[20px] p-4 flex justify-between items-center"><div><p className="text-xs font-bold">My Posts</p><p className="text-[10px] text-white/50">{posts.filter(p=>p.user_id===user?.id).length} posts</p></div><button onClick={()=>setTab('discover')} className="bg-white text-black px-4 py-2 rounded-full text-[10px] font-black">View</button></div>
