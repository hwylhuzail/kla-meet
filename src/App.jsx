...
export default function App(){
  ...
  const [postForm,setPostForm]=useState({type:'dating', bio:'', interests:'', city:'Arua', lat:null, lng:null, imageFile:null, preview:''})
  ...
  const getLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition((pos)=>{
      setPostForm(f=>({...f, lat:pos.coords.latitude, lng:pos.coords.longitude, city:`${pos.coords.latitude.toFixed(2)}, ${pos.coords.longitude.toFixed(2)} (Live)`}))
  ...
  const handleImage = (e) => {
    ...
    if(file){ setPostForm({...postForm, imageFile:file, preview:URL.createObjectURL(file)}) }
  }

  const handleCreatePost = async () => {
    if(!user){ alert('Sign in first'); setView('landing'); return }
    if(!isPremium &&!isAdmin){ setTab('premium'); setShowPostModal(false); alert('Premium required to post'); return }
    try{
      let imageUrl = ''
      if(postForm.imageFile){
        const name = `${user.id}_${Date.now()}_${postForm.imageFile.name}`
        const { error: upErr } = await supabase.storage.from('post-images').upload(name, postForm.imageFile)
        if(upErr){ // fallback to base64 if bucket not exists
          imageUrl = postForm.preview
        } else {
      ...
      setPostForm({type:'dating', bio:'', interests:'', city:'Arua', lat:null, lng:null, imageFile:null, preview:''})
      fetchPosts()
      setTab('discover')
      ...
      <div className="min-h-screen bg-white text-black">
        ...
        <div className="max-w-md mx-auto p-6">
          ...
          <div className="mt-10 space-y-6">
            ...
            <div className="bg-black text-white rounded-[24px] p-5"><h3 className="font-black text-[#FFC300]">How It Works</h3><p className="text-[11px] mt-2">1. Discover 2. Pay $2.99/$5.99 3. Unlock Chat, Near Me, Liked, Profile</p></div>
          ...
          <div className="mt-8 text-[10px] opacity-60 flex gap-4"><span>© KLA-MEET</span></div>
        </div>
      </div>
    )
  }  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white pb-28">
      ...
      {tab==='discover' && (
        <div className="max-w-md mx-auto p-4">
          <h2 className="font-black">Discover • Worldwide {isAdmin && '(Admin sees all)'}</h2>
          {/* User posts */}
          {posts.length>0 && <div className="mt-4 grid grid-cols-2 gap-3">{posts.map(p=>(
            <div key={p.id} className="bg-zinc-900 rounded-[20px] overflow-hidden border border-[#FFC300]/20">
              <img src={p.image_url || WORLD[0].img} className="h-40 w-full object-cover" />
              <div className="p-2"><p className="text-xs font-bold">{p.type==='dating'?'❤️':'🤝'} {p.name} • {p.age}</p><p className="text-[9px] text-white/50">{p.city} • {p.interests}</p><p className="text-[9px] text-white/70 mt-1 truncate">{p.bio}</p>{isAdmin && <button onClick={async()=>{await supabase.from('posts').delete().eq('id',p.id); fetchPosts()}} className="mt-1 text-[8px] bg-red-600 px-2 py-1 rounded-full">Delete Post (Admin)</button>}</div>
            </div>
          ))}</div>}
      ...
      {tab==='nearby' && (isPremium? <div className="max-w-md mx-auto p-4"><h2 className="font-black">Near Me • Arua</h2><div className="mt-4 grid grid-cols-2 gap-3">{posts.filter(p=>p.city).map(p=>(<div key={p.id} className="bg-zinc-900 rounded-[20px] overflow-hidden"><img src={p.image_url || WORLD[0].img} className="h-32 w-full object-cover"/><div className="p-2"><p className="text-xs font-bold">{p.name} • {p.city}</p></div></div>))}</div></div> : <PremiumWall />)}
      ...
      {tab==='profile' && (isPremium? <div className="max-w-md mx-auto p-4"><h2 className="font-black">Profile — {user?.email}</h2><div className="mt-4 bg-zinc-900 rounded-[24px] p-5"><p className="text-xs">Name: {form.name} • {form.gender} • {form.age}</p><p className="text-xs mt-2">Interests: {form.interests}</p><p className="text-xs mt-2">Bio: {form.bio}</p></div></div> : <PremiumWall />)}
      {tab==='premium' && <div className="max-w-md mx-auto p-4 space-y-4"><h2 className="font-black">Premium</h2>{isAdmin && <div className="bg-green-500 text-black rounded-xl p-3 text-xs font-black">Admin: FREE Premium - No need to pay</div>}<div className="bg-[#FFC300] text-black rounded-2xl p-4"><button onClick={openCrypto} className="w-full bg-black text-white rounded-full py-3 font-bold text-xs">Crypto ALONE $2.99 / $5.99</button><p className="text-[7px] mt-1 break-all">{WALLET}</p></div><div className="bg-zinc-900 rounded-2xl p-4"><button onClick={openPesapal} className="w-full bg-[#FF6A00] text-white rounded-full py-3 font-bold text-xs">Pesapal ALONE</button></div><button onClick={()=>{localStorage.setItem('kla_premium','yes'); setIsPremium(true); alert('Premium unlocked for test')}} className="w-full bg-zinc-800 rounded-full py-3 text-xs">TEST PREMIUM FREE</button></div>}

      {tab==='admin' && (
        <div className="max-w-md mx-auto p-4"><h2 className="font-black">Admin Panel</h2><p className="text-[10px] text-white/50">{ADMIN_EMAILS.join(', ')}</p>
          <div className="mt-3 flex gap-2"><button onClick={fetchProfiles} className="bg-zinc-800 px-3 py-1.5 rounded-full text-xs">Refresh</button><button onClick={fetchPosts} className="bg-zinc-800 px-3 py-1.5 rounded-full text-xs">Refresh Posts</button></div>
          <div className="mt-4 space-y-2">{allProfiles.map(p=>(<div key={p.id} className="bg-zinc-900 rounded-xl p-3"><p className="text-xs font-bold">{p.name} • {p.email}</p><div className="flex gap-2 mt-2"><button onClick={()=>setEditData(p)} className="bg-white text-black px-3 py-1 rounded-full text-[10px]">Edit</button><button onClick={async()=>{await supabase.from('profiles').delete().eq('id',p.id); fetchProfiles()}} className="bg-red-600 px-3 py-1 rounded-full text-[10px]">Delete</button></div></div>))}</div>
          {editData && <div className="fixed inset-0 bg-black/90 p-4 flex items-center justify-center z-50"><div className="bg-zinc-900 rounded-2xl p-5 w-full max-w-md"><input value={editData.name||''} onChange={e=>setEditData({...editData,name:e.target.value})} className="mt-3 w-full bg-black border border-white/10 rounded-full px-3 py-2 text-xs" /><div className="flex gap-2 mt-3"><button onClick={async()=>{await supabase.from('profiles').update({name:editData.name}).eq('id',editData.id); setEditData(null); fetchProfiles()}} className="flex-1 bg-[#FFC300] text-black rounded-full py-2 text-xs font-black">Save</button><button onClick={()=>setEditData(null)} className="flex-1 bg-zinc-800 rounded-full py-2 text-xs">Cancel</button></div></div></div>}
        </div>
      )}

      {/* BIG + BUTTON MODAL */}
      {showPostModal && (
        <div className="fixed inset-0 bg-black/90 p-4 flex items-end justify-center z-[100]">
          <div className="bg-zinc-900 rounded-t-[32px] p-6 w-full max-w-md max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center"><h3 className="font-black">Create Post • Dating or Friends</h3><button onClick={()=>setShowPostModal(false)} className="bg-zinc-800 w-8 h-8 rounded-full text-xs">✕</button></div>
            ...
            <div className="mt-4">
              ...
              <input type="file" accept="image/*" onChange={handleImage} className="mt-2 w-full text-xs" />
              {postForm.preview && <img src={postForm.preview} className="mt-3 w-full h-48 object-cover rounded-2xl" />}
            </div>

            <textarea value={postForm.bio} onChange={e=>setPostForm({...postForm,bio:e.target.value})} placeholder="Bio - What are you looking for?" className="mt-4 w-full bg-black border border-white/10 rounded-2xl px-4 py-3 text-xs h-20" />
            <input value={postForm.interests} onChange={e=>setPostForm({...postForm,interests:e.target.value})} placeholder="Interests (Music, Travel...)" className="mt-2 w-full bg-black border border-white/10 rounded-full px-4 py-3 text-xs" />

            <div className="mt-3 bg-black rounded-2xl p-3 border border-white/10">
              <div className="flex justify-between items-center"><p className="text-xs font-bold">📍 Location {postForm.lat && <span className="text-green-400">• Active</span>}</p><button onClick={getLocation} className="bg-[#FFC300] text-black px-4 py-1.5 rounded-full text-[10px] font-black">Activate Location</button></div>
              <p className="text-[10px] text-white/50 mt-2">City: {postForm.city} {postForm.lat && `• ${postForm.lat.toFixed(4)}, ${postForm.lng.toFixed(4)}`}</p>
            </div>

            <button onClick={handleCreatePost} className="mt-5 w-full bg-[#FFC300] text-black rounded-full py-4 font-black text-sm">Post Now • Keep Love Alive</button>
          </div>
        </div>
      )}
      ...
      {/* BOTTOM NAV WITH BIG + */}
      <nav className="fixed bottom-0 left-0 right-0 bg-black border-t border-white/10 flex justify-around items-center py-2 px-2">
        <button onClick={()=>handleTab('discover')} className="text-[11px] flex flex-col items-center"><span>♡</span><span className="text-[9px]">Discover</span></button>
        <button onClick={()=>handleTab('nearby')} className="text-[11px] flex flex-col items-center"><span>◎</span><span className="text-[9px]">Near Me</span></button>

        <button onClick={()=>setShowPostModal(true)} className="bg-[#FFC300] text-black w-14 h-14 rounded-full flex items-center justify-center font-black text-2xl shadow-lg shadow-[#FFC300]/30 -mt-6 border-4 border-black">+</button>

        <button onClick={()=>handleTab('chat')} className="text-[11px] flex flex-col items-center"><span>💬</span><span className="text-[9px]">Chat</span></button>
        {isAdmin? <button onClick={()=>handleTab('admin')} className="text-[11px] text-[#FFC300] flex flex-col items-center font-black"><span>★</span><span className="text-[9px]">Admin</span></button> : <button onClick={()=>handleTab('premium')} className="text-[11px] flex flex-col items-center"><span>★</span><span className="text-[9px]">Premium</span></button>}
      </nav>
    </div>
  )
}